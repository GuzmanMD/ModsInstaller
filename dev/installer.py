...
# Pon aquí el ID del archivo de Google Drive (NO la URL entera)
DRIVE_FILE_ID = "1qVh84Ew_Ksilzwl9IiBzWxsaVDQZPKv8"
DRIVE_URL = f"https://drive.google.com/uc?id=1qVh84Ew_Ksilzwl9IiBzWxsaVDQZPKv8"
...
def seleccionar_mods_final(mods_dir, ventana_padre):
    ...
    ventana_mods.protocol("WM_DELETE_WINDOW", cerrar_todo)
...
def descargar_mods(mods_dir, ventana, barra_progreso):
    try:
        print(f"Descargando desde Google Drive: {DRIVE_URL}")

        # Descargar archivo desde Google Drive con gdown
        output = os.path.join(mods_dir, "mods.zip")
        ...
        os.remove(output)  # eliminar el zip descargado
...
def abrir_github(url="https://github.com/"):
    webbrowser.open(url)
...
def abrir_ventana_juego(nombre_juego):
    ventana = tk.Toplevel()
    ...
    def seleccionar_carpeta():
        ...
        if carpeta:
            ...
            if not os.path.exists(mods_dir):
                ...
                return
            hilo = threading.Thread(target=descargar_mods, args=(mods_dir, ventana, barra_progreso))
            hilo.start()
...
def abrir_menu_juegos():
    ...
    juegos = ["Lethal Company", "REPO", "Phasmo"]

    for nombre in juegos:
        tk.Button(ventana_menu, text=nombre, bg="#7289da", fg="white", font=("Arial", 12, "bold"),
                  relief="raised", bd=3, command=lambda n=nombre: [ventana_menu.destroy(), abrir_ventana_juego(n)]).pack(pady=5)
